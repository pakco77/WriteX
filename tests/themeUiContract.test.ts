...
test("WriteX branding changes user-facing copy but preserves compatibility identifiers", async () => {
  ...
  assert.deepEqual([metadata.id, metadata.name, metadata.version], ["writex", "WriteX", "0.5.6"]);
  ...
  assert.equal(JSON.parse(pkg).version, "0.5.6");
  assert.equal(lock.name, "writex");
  assert.equal(lock.version, "0.5.6");
  assert.equal(lock.packages[""].name, "writex");
  assert.equal(lock.packages[""].version, "0.5.6");
