...
本仓库当前源码为 v0.6.0；可下载安装包仍为上面的 v0.5.8 Release，v0.6.0 的 Release 资产将在单独的发布流程中提供。
...
- **选题评级**：用你明确选择的账号定位 Markdown 分析选题与账号的匹配度，显示可展开的五星评级、依据、时间和模型；分析模型单独选择，主动选择不会因模型目录刷新而改变。
